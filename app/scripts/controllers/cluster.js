    ...
    define(['lodash', 'helpers/cluster-helpers', 'c3'], function(_, ClusterHelpers, c3) {
        ...
        var ClusterController = function($scope, $location, $interval, ClusterService, ServerService, VolumeService, PoolService) {
            ...
            function getRandomNumber() {
                ...
                {
                   resultData.color = '#39a5dc';
                }else if(26<=randomNumber && randomNumber<=50)
                {
                   resultData.color = '#228B22';
                }else if(51<=randomNumber && randomNumber<=75)
                {
                   resultData.color = '#DEB887';
                }else if(76<=randomNumber && randomNumber<=99)
                {
                   resultData.color = '#008B8B';
                }
                return resultData;
            }
                ...
                ClusterService.getList().then(function(clusters){
                    _.each(clusters, function(cluster) {
                        var tempCluster = {
                            ...
                            gauge_cols : [ { id:1, name: 'Used', color: getRandomNumber().color, type: 'gauge' }],
                            gauge_values : [{ '1': getRandomNumber().percentage }]
                        };
